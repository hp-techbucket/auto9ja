...
class Account extends CI_Controller {
		...
		*/		
		public function dashboard(){
			...
			if($this->session->userdata('logged_in')){ 
					...
					$activation = '
								<div class="alert alert-danger">
									<div class="row">
										...
										</h4><p>You cannot make any purchases at this time, please contact <a title="Send Message to Customer Support" data-toggle="modal" data-target="#newMessageModal" style="cursor:pointer; ">Customer Support</a></p>
										</div>
									</div>
								</div>';
				}
				...
				//DISPLAY PENDING PAYMENTS SUMMARY
				$data['pending_payments_array'] = $this->Orders->get_orders($email_address, 'Pending', 5, 0);
				
				//DISPLAY SHORT WATCHLIST
				$data['watchlist_array'] = $this->Watchlist->get_watchlist_summary($email_address);
				
				//GET COUNT OF USER'S ITEMS IN WATCHLIST
				...
				//get shipping status summary
				$data['shipping_array'] = $this->Shipping_status->get_shipping($email_address);
				...
				$data['page_breadcrumb'] = '<i class="glyphicon glyphicon-dashboard"></i> My Dashboard';
				...
				//load main body
				$this->load->view('customer_account_pages/user_dashboard_page', $data);
		...
		public function profile(){
			...
			if($this->session->userdata('logged_in')){
				...
				*/
				...
				//assign additional breadcrumb
				$data['breadcrumb'] = '<li><a href="javascript:void(0)" onclick="location.href=\''.base_url('account/settings/').'\'" title="Private Inbox" ><i class="glyphicon glyphicon-cog"></i> Settings</a></li>';
				...
				//load main body
				$this->load->view('customer_account_pages/user_profile_page', $data);  			
		...
		*/			
		...
		public function settings(){
			...
			if($this->session->userdata('logged_in')){
				...
				//assign additional breadcrumb
				$data['breadcrumb'] = '<li><a href="javascript:void(0)" onclick="location.href=\''.base_url('account/profile/').'\'" title="Private Inbox" ><i class="fa fa-user"></i> Profile</a></li>';
				...
				//load main body
				$this->load->view('customer_account_pages/account_settings_page', $data);  			
		...
		public function update_security(){
			...
			if ($this->form_validation->run()){
				...
				if ($this->Customers->update_customer($data)){
					...
					//send email alert
					$to = $email_address;
					$subject = 'Changes to your account';
					$message = "Your security information has been updated. ". "\n";
					$message .= "If you did not make this change. Please contact us immediately. ";
		...
		public function security_info_validation(){
			...
			if ($this->form_validation->run()){
					...
					if ($this->Customers->update_customer($data)){
						...
						$message = "Your memorable information has been updated. ". "\n";
						$message .= "If you did not make this change. Please contact us immediately. ";
		...
		public function watchlist(){
			...
			if($this->session->userdata('logged_in')){
				...
				if($this->input->get('search') != ''){
						...
						$data['display_option'] = 'Showing Results for "<strong><em>'.$search.'</em></strong>" <a href="'.base_url("account/statements").'"  >Show All</a>';
						
						$config["base_url"] = base_url("account/watchlist/$search");
					...
					$config["base_url"] = base_url("account/watchlist");
				...
				//load main body
				$this->load->view('customer_account_pages/watchlist_page', $data);				
		...
		public function statements(){
			...
			if($this->session->userdata('logged_in')){
				...
				if($this->input->get('search') != ''){
					...
					$config["base_url"] = base_url("account/statements/$search");
					$config["total_rows"] = $count;
					$config["per_page"] = 10;
					$config["uri_segment"] = 3;
					$choice = $config["total_rows"] / $config["per_page"];
					...
					$data['display_option'] = '<strong>Showing All</strong>';
					$config["base_url"] = base_url("account/statements");
				...
				//load main body
				$this->load->view('customer_account_pages/statements_page', $data);				
		...
		public function payments(){
			...
			if($this->session->userdata('logged_in')){
				...
				if($this->input->get('search') != ''){
					...
					$search = html_escape($this->input->get('search'));
					$search = ($this->uri->segment(3)) ? $this->uri->segment(3) : $search;
					$count = $this->Orders->count_status_search($search, $status);
					if($count == '' || $count == null){
						$count = 0;
					}
					...
					$data['display_option'] = 'Showing Results for "<strong><em>'.$search.'</em></strong>" <a href="'.base_url("account/payments").'"  >Show All</a>';
						
					$config["base_url"] = base_url("account/payments/$search");
					$config["total_rows"] = $count;
					$config["per_page"] = 10;
					$config["uri_segment"] = 3;
					$choice = $config["total_rows"] / $config["per_page"];
					...
					$data['my_payments_array'] = $this->Orders->status_search($search, $status, $config["per_page"], $offset);	
					...
					$config["total_rows"] = $this->Orders->count_orders($email_address, $status);
					$config["per_page"] = 10;
					$config["uri_segment"] = 3;
					$choice = $config["total_rows"] / $config["per_page"];
					$config["num_links"] = round($choice);
					...
					//call the model function to get the payments data
					$data['my_payments_array'] = $this->Orders->get_orders($email_address, $status, $config["per_page"], $offset);	
						
					$data["count"] = $this->Orders->count_orders($email_address, $status);
				}
				...
				//load main body
				$this->load->view('customer_account_pages/payments_page', $data);				
		...
		*/			
		public function pending_payments(){
			...
			if($this->session->userdata('logged_in')){
				...
				if($this->input->get('search') != ''){
					...
					$data['display_option'] = 'Showing Results for "<strong><em>'.$search.'</em></strong>" <a href="'.base_url("account/pending_payments").'"  >Show All</a>';
						
					$config["base_url"] = base_url("account/pending_payments/$search");
					$config["total_rows"] = $count;
					$config["per_page"] = 10;
					$config["uri_segment"] = 3;
					$choice = $config["total_rows"] / $config["per_page"];
					...
					$data['display_option'] = '<strong>Showing All</strong>';
					$config["base_url"] = base_url("account/pending_payments");
				...
				//load main body
				$this->load->view('customer_account_pages/pending_payments_page', $data);				
		...
		/**
		* Function to display unpaid orders
		* history
		*/			
		public function unpaid_payments(){
			...
			if($this->session->userdata('logged_in')){
				...
				$data['page_breadcrumb'] = '<i class="fa fa-exclamation-circle"></i> Unpaid Payments';
				
				//assign page title name
				$data['pageTitle'] = 'Unpaid Payments';
				
				//assign page ID
				$data['pageID'] = 'unpaid_payments';
									
				//load header
				$this->load->view('customer_account_pages/header', $data);
								
				//load main body
				$this->load->view('customer_account_pages/unpaid_payments_page', $data);				
		...
		public function shipping(){
			...
			if($this->session->userdata('logged_in')){
				...
				if($this->input->get('search') != ''){
					...
					$data['display_option'] = 'Showing Results for "<strong><em>'.$search.'</em></strong>" <a href="'.base_url("account/shipping").'"  >Show All</a>';
						
					$config["base_url"] = base_url("account/shipping/$search");
					$config["total_rows"] = $count;
					$config["per_page"] = 10;
					$config["uri_segment"] = 3;
					$choice = $config["total_rows"] / $config["per_page"];
				...
				//load main body
				$this->load->view('customer_account_pages/shipping_page', $data);				
		...
		*/			
		public function billing(){
			...
			if($this->session->userdata('logged_in')){
				...
				$get_paypal = $this->Paypal_accounts->get_paypal($email_address);	
				...
				if($get_paypal){
					foreach($get_paypal as $paypal){
						...
						$paypal_id = $paypal->id;
						$masked_paypal = $this->Paypal_accounts->email_mask($paypal->PayPal_email);
				...
				//$data['paypal_array'] = $this->Paypal_accounts->get_paypal($email_address);	
				...
				//load main body
				$this->load->view('customer_account_pages/billing_methods_page');				
		...
		*/
		public function password_reset(){
				...
				//load main body
				$this->load->view('customer_account_pages/password_recovery_page', $data);  							
		}
		...
		*/		
		public function password_recovery_validation(){
			...
			if ($this->form_validation->run()){
				...
				//redirects to confirm_security page
				redirect('account/confirm_security');				
		...
		*/			
		public function validate_email(){
			...
			if ($this->Customers->email_exists()){
				return TRUE;
			}
			else {
		...
		*/			
		public function validate_answer(){
			...
			if ($this->Customers->answer_exists()){
		...
		*/			
		public function new_password(){
			...
			if($this->session->userdata('email_address')){ 
				...
				//load main body
				$this->load->view('customer_account_pages/update_password_page');
			}
			else {
		...
		public function new_password_validation(){
			...
			if ($this->form_validation->run()){
				...
				if($this->Customers->update_customer($data)){
					...
					//send email alert
					$to = $email_address;
					$subject = 'Changes to your account password';
					...
					redirect('account/password_updated');	
		...
		*/	
		public function password_updated(){
			...
			//load main body
			$this->load->view('customer_account_pages/update_password_success');
